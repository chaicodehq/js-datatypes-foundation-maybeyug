...
export function splitAndJoinRangoli(colorString, oldSep, newSep) {
  ...
  const splitColorString = colorString.split(oldSep);
  const joinColorString = splitColorString.join(newSep);
  return joinColorString;
}
...
export function makeRangoliBorder(char, length) {
  ...
  const repeatingChar = char.repeat(20);
  const sliceRepeatingChar = repeatingChar.slice(0, length);
  return sliceRepeatingChar;
}
